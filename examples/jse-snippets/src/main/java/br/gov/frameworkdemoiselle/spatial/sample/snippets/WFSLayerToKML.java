package br.gov.frameworkdemoiselle.spatial.sample.snippets;
...
public class WFSLayerToKML {
	...
	public static void main(String[] args) throws MalformedURLException, ServiceMetadataLookupException, WFSClientGetFeatureException {
		...
		kmlBuilder.buildKmlAsFileFromSimpleFeature(features, new File("/Users/otos/development/projeto/demoiselle-spatial/tmp/WFS_SIPAM.kml"));
		
		List<SimpleFeature> features2 = wfsClient.getFeature(service, service.getCapability().getWfs().getLayers().get(2));
		
		kmlBuilder.buildKmlAsFileFromSimpleFeature(features2, new File("/Users/otos/development/projeto/demoiselle-spatial/tmp/WFS_SIPAM_2.kml"));
