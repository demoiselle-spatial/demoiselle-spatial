...
@ViewController
@NextView("/lote_edit.xhtml")
@PreviousView("/lote_list.xhtml")
public class LoteListMB extends AbstractListPageBean<Lote, Long>{
	...
	@Override
	protected List<Lote> handleResultList() {
	...
	public String getResultFeatureList() throws ParseException {		
		 ...
		 List<Lote> beans =  bc.finAllByExtent(new EnvelopeWrapper("-73.125, -25.3125,133.59375, 40.078125",4326).getEnvelope()); //this.getResultList();
		 WKTFeaturesCollection wktFeatures = new WKTFeaturesCollection();
		 FeatureVector fv;
		 
		//List<Contact> beans = bc.findAll();
		
		 for (Lote client : beans) {
	...
	{		
		...
		List<Lote> buildings = new ArrayList<Lote>();
